...
import com.ms.coco.registry.http.HttpGetHolder;
import com.ms.coco.registry.model.ServiceConf;
import com.ms.coco.registry.simple.SimpleModeCenter;
...
public class RegisterClient {
    ...
    public static void deleteService(ServiceConf serviceConf) throws Exception {
        CuratorFramework client = RegisterHolder.getClient(serviceConf.getRegisterUrl());
        String groupPath = CocoUtils.buildPath(serviceConf.getServiceName(), NodeEnum.GROUP_NODE.value(),
                serviceConf.getGroupName());
        String serverPath = groupPath + "/" + serviceConf.getServerNode().hostKey();
        LOGGER.info("start to [delete] to path [{}]", serverPath);
        CocoUtils.deleteNode(client, serverPath);
        LOGGER.info("end to [delete] to path [{}]", serverPath);
    }
